...
-- Evidence links (ingredient_compounds, compound_literature, product_sources). These were
-- in supabase/migrations but never folded in here, so the deployed database never got them
-- and a grant on compound_literature aborted the whole migration.

...
// Greek letters must be spelled out, not stripped: γ-Cadinene and δ-Cadinene are
// different sesquiterpenes and would otherwise collapse onto the same compound id.
const slug = (value) => value.toLowerCase().replaceAll("β", "beta-").replaceAll("α", "alpha-").replaceAll("γ", "gamma-").replaceAll("δ", "delta-").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
const id = (prefix, value) => `${prefix}-${slug(value)}`;
const rows = { products: [], ingredients: [], links: [], coas: [], compoundLinks: [], results: [], sources: [], images: [] };
const compounds = new Map(); // compound_id -> display name; measured names win over candidates.
...
for (const product of data.products) {
  ...
  if (profile) {
    ...
    for (const compound of profile.compounds) {
      ...
      compounds.set(compoundId, compound.name);
    ...
    for (const molecule of product.molecules || []) {
      ...
      if (!compounds.has(compoundId)) compounds.set(compoundId, molecule);
...
const compoundRows = [...compounds].map(([compoundId, name]) => `(${esc(compoundId)},${esc(name)},NULL,${esc("terpene")},NULL,CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP())`);
const sql = `
...
INSERT INTO \`${project}.terpedia_ops.terproduct_compounds\` VALUES ${compoundRows.join(",")};
